...
public class Random_Number_Guess_Game {
	...
	/*
	 * This function is used to generate a 4 digit unique number and store it in an
	 * array. this function also returns the 4 digit unique number in array format.
	 */
...
//this line is used to generate single digit random number
	...
	/*In this method we give the random number as input and also get input from the 
	 * user to compare the user number with our secret number to validate the user number.*/
...
/*Once we have the user number as an array we have to take each digit of the user number
 * and compare it witht the secret number.
 * 
 * if the first digit of the user number matches the first digit of the secret number we can note it 
 * by using another char array and store first value of the array as 'C'.
 * 
 * if we find that the first digit of the user number is present in the secret number but the position is
 * different, then we store the 'P' into the char array.
 * 
 * if the first digit of the user number is not present in the secret number then we can store 'X' into 
 * the char array.
 * 
